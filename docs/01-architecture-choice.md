...
## Choice made
After weighing the pros and cons of each architecture, I think I will use the last one presented. After thorough analysis, it will probably not be possible to write directly to the engine's VkBuffer, but it will be necessary to use a temporary VkBuffer and make a GPU-to-GPU copy. This is not optimal, but it is the best solution found without having to fork the engine and create custom methods.